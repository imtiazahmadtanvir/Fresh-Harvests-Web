...
const CONTACT_INFO: ContactInfo = {
  ...
  address: "TunjukStreet,Pontianak,Indonesia",
}
...
const PaymentMethod = React.memo(({ name }: { name: string }) => (
...
const SocialIcon = React.memo(({ href, icon, label }: { href: string; icon: React.ReactNode; label: string }) => (
  <Link
    href={href}
    className="w-8 h-8 bg-gray-800 text-white rounded-full flex items-center justify-center hover:bg-green-600 transition-colors duration-200"
    aria-label={label}
  >
    {icon}
  </Link>
  ...
  return (
    <footer className="bg-gray-100 py-8 lg:py-12">
      <div className="container mx-auto px-6">
          ...
          <div>
            ...
            <ul className="space-y-3">
              {QUICK_LINKS_1.map((link) => (
                <li key={link.label}>
                  <Link href={link.href} className="text-gray-600 hover:text-green-600 transition-colors duration-200">
                    {link.label}
                  </Link>
                </li>
              ))}
        ...
        <div className="lg:hidden mt-8 pt-8 border-t border-gray-300">
          <p className="text-gray-700 font-medium mb-4">Download App:</p>
